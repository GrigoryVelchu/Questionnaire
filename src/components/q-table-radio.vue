<template>
    <div class="quest">
        ...
        <div class="values_header">
          <div v-for="value in quest.values" :key="value">{{value}}</div>
        </div>
        <div class="quest__input_container" v-for="type in quest.types" :key="type">
  ...
  export default {
    ...
    methods:{
      dataHandler(e, type, value){
        ...
        this.$emit('checkboxHandler',this.answers,this.currentQuest)
      }
    },
    mounted() {
      this.answers = this.quest.types.map(el=>{
        return {type:el,value:''}
      })
    }
  }
</script>
